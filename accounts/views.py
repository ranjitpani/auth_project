...
def place_order(request):
    ...
    if checkout_type == 'buy_now' and request.session.get('buy_now'):
        ...
        price = product.discounted_price or product.price
        product_delivery = product.delivery_charge or 0

        # 🔥 STORE LOCATION
        store_lat = product.store.latitude
        store_lng = product.store.longitude

        km_charge = calculate_km_delivery_charge(
        ...
        subtotal += price * qty
        delivery_total += final_delivery * qty
...
#     # 🔥 USER LOCATION (FROM SESSION)
#     user_lat = request.session.get("order_latitude")
#     user_lng = request.session.get("order_longitude")
...
def checkout_summary(request):
    ...
    if checkout_type == 'buy_now' and request.session.get('buy_now'):
        ...
        store_lat = product.store.latitude
        store_lng = product.store.longitude
        km_charge = calculate_km_delivery_charge(store_lat, store_lng, user_lat, user_lng)
    ...
    discount = original_total - total_amount
    grand_total = total_amount + product_delivery_total + km_delivery_total

    # 🔹 SAVE FOR PAYMENT STEP
    request.session['order_total'] = float(grand_total)
    request.session['order_discount'] = float(discount)
    request.session['order_items_count'] = len(items)
    request.session['product_delivery_total'] = float(product_delivery_total)
    request.session['km_delivery_total'] = float(km_delivery_total)
...
#         # 🔥 KM delivery
#         store_lat = getattr(product.store, 'latitude', None)
#         store_lng = getattr(product.store, 'longitude', None)
#         km_delivery = calculate_km_delivery_charge(
#             store_lat, store_lng, user_lat, user_lng
#         )

#         product_delivery_total += product_delivery
#         km_delivery_total += km_delivery
